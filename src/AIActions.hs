...
findSideTile board maxR (col, row) distance colDiff rowDiff = state $ \(visited, found) ->
  if floor (distance + 1) <= maxR
    && validCoord board (col + colDiff, row + rowDiff)
    && not ((visited V.! (row + rowDiff)) V.! (col + colDiff))
    && (board V.! (row + rowDiff)) V.! (col + colDiff) /= Wall
    then
      let newVisRow = (visited V.! (row + rowDiff)) V.// [(col + colDiff, True)]
...
findDiagonalTile board maxR (col, row) distance colDiff rowDiff = state $ \(visited, found) ->
  if floor (distance + 1.5) <= maxR
    && validCoord board (col + colDiff, row + rowDiff)
    && not ((visited V.! (row + rowDiff)) V.! (col + colDiff))
    && (board V.! (row + rowDiff)) V.! (col + colDiff) /= Wall
...
findNewTiles board maxR (pos, distance) = do
...
searchTargets board units maxR pred visited queue =
  if null queue
    then []
    else
      let ((), (newVisited, revCoords)) = runState (findNewTiles board maxR (Seq.index queue 0)) (visited, [])
...
unitsInRange gameState index predicate range =
  let Mob current = (units gameState) V.! index
...
sortByAdjective state index searchUnits Last =
  let targetHistory = getTargets ((units state) V.! index)
...
evalTarget state index range (Specific team name id) =
  let predicate = (\u -> getTeam u == team && getName u == name && getIdentifier u == id)
   in unitsInRange state index predicate range
evalTarget state index range (Description adjective unitDesc) =
  let unitPred = evalUnitDesc state index unitDesc
...
aiStep index (Cons first second) = do
  ...
  case response of
    (None, playedTurn) ->
      if playedTurn
        then return (second, True)
        else do
          put gameState
          aiStep index second
    (newFirst, playedTurn) -> return (Cons newFirst second, playedTurn)
aiStep index (While cond action) = do
  gameState <- get
  if evalCondition gameState index cond
