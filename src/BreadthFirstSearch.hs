...
isTileVisited (_, tileDistance) = tileDistance >= 0
...
findSideTile board goThroughUnits (col, row) colDiff rowDiff = state $ \(visited, found) ->
  if validCoord board (col + colDiff, row + rowDiff)
    ...
    then
      let (_, currentDistance) = (visited V.! row) V.! col
       ...
       in if isUnitTile ((board V.! (row + rowDiff)) V.! (col + colDiff)) && not goThroughUnits
            then ((), (newVisited, found))
            else ((), (newVisited, (col + colDiff, row + rowDiff) : found))
    else ((), (visited, found))
...
findDiagonalTile board goThroughUnits (col, row) colDiff rowDiff = state $ \(visited, found) ->
  if validCoord board (col + colDiff, row + rowDiff)
    && not (isTileVisited ((visited V.! (row + rowDiff)) V.! (col + colDiff)))
...
findNewTiles board goThroughUnits pos = do
  findSideTile board goThroughUnits pos 0 1
  findSideTile board goThroughUnits pos 0 (-1)
  findSideTile board goThroughUnits pos 1 0
  findSideTile board goThroughUnits pos (-1) 0
  findDiagonalTile board goThroughUnits pos 1 1
  findDiagonalTile board goThroughUnits pos 1 (-1)
  findDiagonalTile board goThroughUnits pos (-1) 1
  findDiagonalTile board goThroughUnits pos (-1) (-1)
  return ()
...
expandTiles board distanceMap goThroughUnits queue =
  if null queue
    then distanceMap
    else
      let ((), (newMap, reversedCoords)) = runState (findNewTiles board goThroughUnits (Seq.index queue 0)) (distanceMap, [])
       in expandTiles board newMap goThroughUnits ((Seq.drop 1 queue) Seq.>< (Seq.fromList (reverse reversedCoords)))
